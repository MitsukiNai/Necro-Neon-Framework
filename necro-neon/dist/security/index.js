 ...
 */
export function safeTextNode(raw) {
...
export function validateField(raw, opts = {}) {
    const { required = false, minLength, maxLength, pattern, blockSql = true, blockHtml = true, } = opts;
    const errors = [];
    let value = raw.trim();
...
export function generateCSRFToken() {
    ...
    const store = { token, expires: Date.now() + CSRF_TTL };
    try {
 ...
 */
export function validateCSRFToken(submitted) {
 ...
 */
export function getRecommendedCSP(nonce) {
    return [
        `default-src 'self'`,
