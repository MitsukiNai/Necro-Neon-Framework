...
export class InfestationEngine {
  ...
  constructor(canvas: HTMLCanvasElement, opts: InfestationOptions = {}) {
    ...
    if (!ctx) throw new Error('[NecroNeon] InfestationEngine: 2d context unavailable');
  ...
  destroy(): void { this.stop(); this._resizeObserver.disconnect(); }

  get isActive(): boolean { return this._active; }
  setRate(rate: number): void { this._rate = Math.max(1, Math.min(10, rate)); }
  ...
  private _loop(): void {
    ...
    for (let i = 0; i < 18; i++) {
      ...
      this.ctx.bezierCurveTo(
        x0 + Math.sin(t + i) * 130,
        y0 + Math.cos(t * 1.3 + i) * 90,
        x1 + Math.cos(t * 0.7 + i) * 110,
        y1 + Math.sin(t + i * 0.5) * 100,
        x1, y1
      );
    ...
    if (this._active) this._rafId = requestAnimationFrame(() => this._loop());
  }
}
...
export class GlitchEngine {
  ...
  setIntensity(v: number): void { this.intensity = Math.max(1, Math.min(10, v)); }
  ...
  burst(): void {
    ...
    const els = document.querySelectorAll<HTMLElement>('.nn-card,.nn-btn');
    if (els.length) {
