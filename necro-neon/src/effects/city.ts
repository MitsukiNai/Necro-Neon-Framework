...
import type { DistrictCityColors } from '../core/district.js';
...
export function drawCity(
  ...
  const { sky, mid, neon, glow }: DistrictCityColors = DISTRICT_CITY_COLORS[district];
  ...
  if (style === 'ruined') {
    ...
    ctx.strokeStyle = 'rgba(204,0,24,.25)'; ctx.lineWidth = 0.5;
    for (let i = 0; i < 4; i++) {
      ctx.beginPath();
      ctx.moveTo(mx, my);
  ...
  for (let layer = LAYER_COUNT; layer >= 0; layer--) {
    ...
    while (bx < W + 90) {
      ...
      const bStyle: CityStyle =
        style === 'ruined' && layer < 2 ? 'ruined' : style;
...
export class CityBackdropManager {
  private readonly _wrapMap: Record<string, string> = {
