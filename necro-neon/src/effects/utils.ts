 ...
 */
export function staggerGlitch(
  root: HTMLElement | Document = document,
  stepSeconds = 0.55
): void {
  root.querySelectorAll<HTMLElement>('.glitch').forEach((el, i) => {
 ...
 */
export function addScanlines(): HTMLElement {
 ...
 */
export function addGrain(opacity = 0.03): HTMLElement {
  const el = document.createElement('div');
  el.setAttribute('aria-hidden', 'true');
