...
export class NNTabs {
  ...
  constructor(root: HTMLElement, opts: TabsOptions) {
    ...
    opts.tabs.forEach((tab, i) => {
      ...
      if (i === 0 && !opts.activeId) btn.classList.add('active');
      if (i === 0 && !opts.activeId) panel.classList.add('active');
    });
    ...
    if (opts.activeId) this.activate(opts.activeId);
  }

  activate(id: string): void {
    this._activeId = id;

    this.root.querySelectorAll<HTMLElement>('.nn-tab').forEach((btn) => {
      const active = btn.getAttribute('data-tab') === id;
      btn.classList.toggle('active', active);
      btn.setAttribute('aria-selected', String(active));
    });

    this.root.querySelectorAll<HTMLElement>('.nn-tab-content').forEach((panel) => {
  ...
  static init(): void {
    document.querySelectorAll<HTMLElement>('.nn-tab').forEach((btn) => {
      btn.addEventListener('click', () => {
        const targetId = btn.getAttribute('data-target');
        if (!targetId) return;
        const nav = btn.closest('.nn-tab-nav') ?? document;
        nav.querySelectorAll<HTMLElement>('.nn-tab').forEach((b) => b.classList.remove('active'));
        document.querySelectorAll<HTMLElement>('.nn-tab-content').forEach((p) => p.classList.remove('active'));
