...
export class NNModal {
  constructor() {
    ...
    document.addEventListener('keydown', (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        document.querySelectorAll<HTMLElement>('.nn-modal-overlay.open').forEach((m) => {
  ...
  close(id: string): void {
    ...
    if (el) {
      const opts = (el as HTMLElement & { _nnModalOpts?: ModalOptions })._nnModalOpts;
      opts?.onClose?.();
    }
  }
  ...
  create(opts: ModalOptions): HTMLElement {
    ...
    if (title) overlay.setAttribute('aria-label', escapeHtml(title));
    ...
    (overlay as HTMLElement & { _nnModalOpts?: ModalOptions })._nnModalOpts = opts;
